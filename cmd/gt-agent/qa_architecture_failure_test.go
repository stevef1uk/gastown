...
func TestValidateQAArtifacts_architectureFailure_scenario(t *testing.T) {
	...
	err := validateQAArtifacts(dir, rig, "architecture_failure", true, true, true, false, v)
	if err != nil {
		t.Fatalf("architecture_failure should be allowed: %v", err)
	}

	// Wrong outcome: failure when tests pass but smoke failed.
	err = validateQAArtifacts(dir, rig, "failure", true, true, true, false, v)
	...
	err = validateQAArtifacts(dir, rig, "architecture_failure", false, true, false, false, v)
	...
	err = validateQAArtifacts(dir, rig, "architecture_failure", false, true, true, true, v)
