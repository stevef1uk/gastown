...
func TestValidateImplementWritePath_rejectsSchemaTestWhileOnStoreBead(t *testing.T) {
	...
	if err := orchestrator.ValidateImplementWritePath(dir, rig, "te-store", "linkshelf/internal/store/store_test.go", v, true, "", nil); err != nil {
		t.Fatalf("store bead should allow store_test.go: %v", err)
	}
	// schema_test.go is not correlated to store.go
	if err := orchestrator.ValidateImplementWritePath(dir, rig, "te-store", "linkshelf/internal/store/schema_test.go", v, true, "", nil); err == nil {
