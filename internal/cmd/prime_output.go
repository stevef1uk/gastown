...
func outputStartupDirective(ctx RoleContext) {
	...
	fmt.Println("1. Rule 1: Output EXACTLY ONE command per turn in the format: CMD: [command]")
	fmt.Println("2. Rule 2: NO YAPPING. DO NOT explain your reasoning. DO NOT simulate output.")
	fmt.Println("3. Rule 3: DO NOT simulate subsequent turns or additional commands.")
	fmt.Println("4. Rule 4: If no command is needed, explain in one sentence and wait.")
	fmt.Println()
	fmt.Println("FAILURE TO FOLLOW THESE RULES CAUSES SYSTEM CORRUPTION.")
	...
	case RoleMayor:
		...
		fmt.Println("1. Check mail: `" + cli.Name() + " mail inbox`. If unread → `gt mail read [id]`")
		fmt.Println("2. Check for attached work: `" + cli.Name() + " hook`")
	...
	case RoleWitness:
		...
		fmt.Println("2. Check mail: `" + cli.Name() + " mail inbox`. If unread → `gt mail read [id]`")
	...
	case RoleRefinery:
		...
		fmt.Println("1. Check mail: `" + cli.Name() + " mail inbox`. If unread → `gt mail read [id]`")
		fmt.Println("2. Check for attached patrol: `" + cli.Name() + " hook`")
		fmt.Println("   - If mol attached → **RUN IT** (resume from current step)")
		fmt.Println("   - If no mol → create patrol: `" + cli.Name() + " patrol new`")
	case RoleCrew:
		fmt.Println()
	...
	case RoleDeacon:
		...
		fmt.Println("3. Check mail: `" + cli.Name() + " mail inbox`. If unread → `gt mail read [id]`")
	...
	case RoleBoot:
		...
		fmt.Println("1. Run `" + cli.Name() + " boot triage` immediately")
	...
	case RolePlanner:
		...
		fmt.Println("1. Check for planning beads on your hook: `" + cli.Name() + " hook`")
		fmt.Println("3. If no hook, check mail: `" + cli.Name() + " mail inbox`. If unread → `gt mail read [id]`")
	case RoleArchitect:
		fmt.Println("**STARTUP PROTOCOL**: You are the Architect.")
		fmt.Println("1. If a project is HOOKED above, run `gt sling [id]` IMMEDIATELY to start designing.")
		fmt.Println("3. Do NOT check status repeatedly. START THE WORK.")
	case RoleQA:
		fmt.Println()
		fmt.Println("---")
		fmt.Println()
		...
		fmt.Println("3. If no hook, check mail: `" + cli.Name() + " mail inbox`. If unread → `gt mail read [id]`")
	case RoleMechanic:
		fmt.Println("**STARTUP PROTOCOL**: You are the Mechanic.")
		fmt.Println("1. Run `" + cli.Name() + " prime` (loads context)")
		fmt.Println("2. Run your patrol script: `./patrol.sh` (or equivalent)")
		fmt.Println("3. If you find errors, fix them and nudge the affected agents.")
	}
}
