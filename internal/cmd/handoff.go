...
This is the canonical way to end any agent session. It handles all roles:
  ...
  - Mayor, Crew, Witness, Refinery, Deacon, Planner, Architect, QA: Respawns with fresh Claude instance
...
func collectHandoffState() string {
	...
	// Get hooked work
	hookOutput, err := exec.Command(gtExecutable(), "hook").Output()
	if err == nil {
	...
	inboxOutput, err := exec.Command(gtExecutable(), "mail", "inbox").Output()
	if err == nil {
