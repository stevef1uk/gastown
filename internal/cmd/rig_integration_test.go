...
func checkWorktreeClean(t *testing.T, agent agentWorktree, hasTrackedBeads bool) []string {
	...
	if hasTrackedBeads {
		// EnsureMayorRigGitHygiene untracks the mayor worktree's tracked .beads/
		// from the index (git rm --cached) so orchestrator checkpoint commits
		// don't include beads runtime state. This shows up as staged deletions
		// in the mayor worktree and is expected (rig/overlay.go mayorRigUntrackPaths).
		// Note: porcelain "D " (index) + " " (worktree) stays "D  path" after TrimSpace.
		allowlist["D  .beads/.gitignore"] = true           // Untracked .gitignore (staged deletion)
		allowlist["D  .beads/config.yaml"] = true          // Untracked config.yaml (staged deletion)
