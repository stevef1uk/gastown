...
func TestSessionName(t *testing.T) {
	...
	m := NewSessionManager(session.NewTmuxProvider(tmux.NewTmux(), "/tmp"), r)
...
func TestEnsureCanonicalSessionBranch_UsesOriginDefaultBranch(t *testing.T) {
	...
	sm := NewSessionManager(session.NewTmuxProvider(tmux.NewTmux(), workDir), &rig.Rig{Name: "gastown", Path: workDir})
...
func TestVerifyStartupNudgeDelivery_IdleAgent(t *testing.T) {
	...
	m := NewSessionManager(session.NewTmuxProvider(tm, r.Path), r)
...
func TestPolecatSlot(t *testing.T) {
	...
	sm := NewSessionManager(session.NewTmuxProvider(tmux.NewTmux(), rigPath), r)
