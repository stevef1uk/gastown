...
func TestGoToolArgsFromVerify_shellOperators(t *testing.T) {
	t.Parallel()
	v := DefaultWorkflowValidation()
	v.QAVerifyCommand = "cd pingapp && go build -o server ./cmd/server && (./server & SERVER_PID=$!; sleep 1; curl -s -f http://localhost:8080/ping | grep -q '\"message\":\"pong\"' && kill $SERVER_PID)"
	got := goToolArgsFromVerify(v)
	want := []string{"build", "-o", "server", "./cmd/server"}
	if len(got) != len(want) {
		t.Fatalf("got %v want %v", got, want)
	}
	for i := range want {
