...
func PlanningPlanMDNeedsRefresh(townRoot, rig string, v WorkflowValidation) bool {
	...
	if len(checkPlanBeadMapExactPaths(string(data), v, rig)) > 0 {
		return true
	}
...
func ValidatePlanMDBeadPathAlignment(townRoot, rig string, v WorkflowValidation) error {
	...
	planDoc := string(data)
	if issues := checkPlanBeadMapExactPaths(planDoc, v, rig); len(issues) > 0 {
	...
	for _, line := range strings.Split(planDoc, "\n") {
		...
		got := NormalizePlannerBeadPath(extractPlanBeadMapPath(line), v.LayoutRoot, rig)
		want, ok := idToPath[id]
		if !ok || got == "" {
			continue
		}
...
func WritePlanningPlanMD(townRoot, rig string, v WorkflowValidation) (bool, error) {
	...
	for _, want := range files {
		id := pathToID[want]
		displayPath := NormalizePlannerBeadPath(want, v.LayoutRoot, rig)
