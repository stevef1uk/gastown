...
const RigFlowStaticURLContractGuidance = "Static asset URLs come **only** from **architecture.md** (HTTP routes) and the exact " +
	"`src`/`href` paths in the frontend `index.html`. Implementation, unit tests, and QA smoke must use the **same** URLs. " +
...
const RigFlowStaticURLContractShort = "URLs from **architecture.md** + the frontend `index.html` only (gt-agent smoke uses those paths — do not guess `/app.js` vs `/static/…`)."
